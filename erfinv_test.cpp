...
void test_out_of_domain_values() {
  ASSERT_NAN(erfinv(-2.0L));
  ASSERT_INF(erfinv(-1.0L), -1);
  ASSERT_INF(erfinv(1.0L), 1);
  ASSERT_NAN(erfinv(2.0L));
}
...
#define ASSERT_ERFINV_WITH_NR(table_entry)                                     \
  {                                                                            \
    long double x, y, eps;                                                     \
    tie(x, y, eps) = table_entry;                                              \
    assert(fabsl(erfinv_refine(x, 1) - y) < 0.1 * eps);                        \
  }
